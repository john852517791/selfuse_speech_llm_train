...
class base_model(L.LightningModule):
    def __init__(self, 
        ...
        for param in self.encoder.parameters():
            param.requires_grad = False
        prompt_tune_conf = conf_yaml["prompt_tune_conf"]
        self.prompt_embeddings = nn.Embedding(prompt_tune_conf["prompt_num"], adaptor_conf["llm_embed_dim"])
        self.prompt_ids = torch.Tensor([i for i in range(prompt_tune_conf["prompt_num"])]).long().unsqueeze(0).expand(self.conf.basic_settings.batch_size, -1)

        
    def forward(self,batch):
        fbank, fbank_len, target, target_len,texts = batch
        special_token_emb = self.prompt_embeddings(self.prompt_ids.to(target.device)).to(target.device)
        ...
        all_inputs_embeds = torch.cat([special_token_emb,inputs_embeds, outputs_embed], dim=1)
        # 3、拼接attention mask
        pretend_att_mask = torch.ones(self.prompt_ids.shape, dtype=torch.bool).unsqueeze(1).to(target.device)
        all_attmask = torch.cat([pretend_att_mask, encoder_mask, target_len_mask], dim=-1).squeeze(1)
        # 4、构建labels
        labels_inputs = torch.full((inputs_embeds.size(0), inputs_embeds.size(1)), -100, dtype=torch.long).to(target.device)
        
        # neglect the prompt loss
        # labels_pretend = torch.full((self.prompt_ids.size(0), self.prompt_ids.size(1)), -100, dtype=torch.long).to(target.device)
        # labels = torch.cat([labels_pretend,labels_inputs, target], dim=-1)
        
        # culculate the prompts loss
        labels = torch.cat([self.prompt_ids.to(target.device),labels_inputs, target], dim=-1)
    ...
    def configure_optimizers(self):
        ...
        optimizer_params = [
            ...
            {'params': self.prompt_embeddings.parameters(), 'lr': lr, 'weight_decay': weight_decay},
        ]
        optim = torch.optim.AdamW(optimizer_params)
