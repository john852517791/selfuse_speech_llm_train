...
def get_args():
    ...
    parser.add_argument('--model_path', default='Freeze-Omni/checkpoints')
    parser.add_argument('--llm_path',  default='Qwen2-7B-Instruct')
    parser.add_argument('--top_p', type=float, default=0.8)
    parser.add_argument('--top_k', type=int, default=20)
    parser.add_argument('--temperature', type=float, default=0.8)
    parser.add_argument('--input_wav', default="./assets/question.wav")
    parser.add_argument('--output_wav', default="./op.wav")
...
def inference(pipeline, audio_processor, tts, configs):
    ...
    while True:
        ...
        if outputs['stat'] == 'cs':
            ...
            suffix_list = ["。", "：", "？", "！", ".", "?","!", "\n"]
            if outputs['text'][len(last_text):].endswith(tuple(suffix_list)): # 是否预测到上述标点符号
