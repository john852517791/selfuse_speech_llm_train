...
class CrossEntropyLoss(torch.nn.Module):
    ...
    def teacher_force_forward(self, logits, target):
        ...
        shift_logits = shift_logits.view(-1, 1024)
...
class LLM2TTSCodecAR(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.encoder_criterion == 'ce':
            loss_ori = self.criterion(logits, y)
            # loss_tf = self.criterion.teacher_force_forward(logits, y)
    ...
    def infer(self, hidden, top_k, prefix, penalty_window_size, penalty, max_tokens=1000):
        ...
        past_key_values = DynamicCache.from_legacy_cache(None)
        keycache0 = past_key_values.key_cache
        ...
        keycache1 = past_key_values.key_cache
