...
def make_pad_mask(ilens, max_len):
    # 创建一个布尔掩码
    batch_size = len(ilens)
    # mask = torch.arange(max_len).expand(batch_size, max_len) >= ilens.unsqueeze(1)
    device = ilens.device  # 获取 ilens 的设备
    # 将 mask 移动到 ilens 所在的设备
    mask = torch.arange(max_len).expand(batch_size, max_len).to(device) >= ilens.unsqueeze(1)
    return mask